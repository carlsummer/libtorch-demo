...
ICudaEngine* createEngine_s(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("./weights/solarcell.wts");
...
int main(int argc, char** argv) {
    ...
    engine_name = "solarcell" + engine_name + ".engine";
    ...
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::cerr << "./yolov5-tensorrt -s  // serialize model to plan file" << std::endl;
        std::cerr << "./yolov5-tensorrt -d ../samples  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
