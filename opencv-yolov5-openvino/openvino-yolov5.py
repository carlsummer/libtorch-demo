...
def build_argparser():
    ...
    parser.add_argument(
        "--model-xml",
        type=str,
        default=r"C:\Users\zengxh\Documents\workspace\visualstudio-workspace\libtorch-demo\libtorch-yolov5-openvino\weights\solarcell.xml",
        help="model.xml path",
    )
    parser.add_argument(
        "--image-path",
...
def main(args):
    ...
    key = list(outputs.keys())[-1] # key 要为output  1,25200,10 的才对
    output = outputs[key]
    end = time.time()
